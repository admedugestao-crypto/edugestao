...
6. Body: rotas que recebem upload de arquivo ou formulários grandes usam `req.formData()` (ex.: `/api/alunos`); rotas mais simples usam `req.json()` (ex.: `/api/pagamentos/gerar`, `/api/biblioteca`). Siga o padrão já usado por rotas irmãs do mesmo recurso.
