  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-5">
      <div className="max-w-md">
        ...
        <select value={professoraId} onChange={(e) => { setProfessoraId(e.target.value); setMensagem(""); }} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm bg-white">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <button type="button" onClick={() => alterar([...(professora?.disponibilidade ?? []), { dia: "Segunda", inicio: "08:00", fim: "12:00" }])} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-indigo-50 text-indigo-700 text-sm font-medium hover:bg-indigo-100">
      ...
      {professora?.disponibilidade.length === 0 ? (
        ...
        <div className="space-y-2">
          {professora?.disponibilidade.map((h, i) => (
            <div key={`${h.dia}-${i}`} className="grid grid-cols-[minmax(120px,1fr)_120px_auto_120px_36px] items-center gap-2 bg-slate-50 rounded-lg p-2">
              <select value={h.dia} onChange={(e) => alterar(professora.disponibilidade.map((item, j) => j === i ? { ...item, dia: e.target.value } : item))} className="border border-slate-200 rounded-lg px-2 py-2 text-sm bg-white">
                {DIAS.map((dia) => <option key={dia}>{dia}</option>)}
              </select>
              <input type="time" value={h.inicio} onChange={(e) => alterar(professora.disponibilidade.map((item, j) => j === i ? { ...item, inicio: e.target.value } : item))} className="border border-slate-200 rounded-lg px-2 py-2 text-sm" />
              <span className="text-sm text-slate-400">até</span>
              <input type="time" value={h.fim} onChange={(e) => alterar(professora.disponibilidade.map((item, j) => j === i ? { ...item, fim: e.target.value } : item))} className="border border-slate-200 rounded-lg px-2 py-2 text-sm" />
              <button type="button" aria-label="Remover horário" onClick={() => alterar(professora.disponibilidade.filter((_, j) => j !== i))} className="text-slate-400 hover:text-red-600 flex justify-center"><Trash2 size={16} /></button>
            </div>
          ))}
        </div>
      )}

      {mensagem && <p className={`text-sm rounded-lg px-3 py-2 ${mensagem.includes("sucesso") ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600"}`}>{mensagem}</p>}
