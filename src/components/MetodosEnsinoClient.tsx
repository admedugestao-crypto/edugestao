...
export default function MetodosEnsinoClient({
  metodosIniciais,
}: {
  metodosIniciais: MetodoEnsino[];
}) {
  const [metodos, setMetodos] = useState(metodosIniciais);
  const [modal, setModal] = useState(false);
  const [novoNome, setNovoNome] = useState("");
  const [salvando, setSalvando] = useState(false);
  const [erro, setErro] = useState("");

  const [editando, setEditando] = useState<MetodoEnsino | null>(null);
  const [erroEdit, setErroEdit] = useState("");

  const [confirmDelete, setConfirmDelete] = useState<MetodoEnsino | null>(null);
  ...
  async function criarMetodo() {
    ...
    setErro("");
    try {
      const res = await fetch("/api/metodos-ensino", {
        method: "POST",
      ...
      if (!res.ok) {
        setErro(data.erro ?? "Erro ao criar método.");
        return;
      }
      setMetodos((prev) => [...prev, data].sort((a, b) => a.nome.localeCompare(b.nome)));
      setModal(false);
      setNovoNome("");
    } finally {
      setSalvando(false);
  ...
  async function salvarEdicao() {
    ...
    setErroEdit("");
    try {
      const res = await fetch(`/api/metodos-ensino/${editando.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      setMetodos((prev) => prev.map((m) => (m.id === data.id ? data : m)).sort((a, b) => a.nome.localeCompare(b.nome)));
      setEditando(null);
    } finally {
      setSalvando(false);
    }
  ...
  async function excluirMetodo() {
    if (!confirmDelete) return;
    setSalvando(true);
    setErroDelete("");
    try {
      const res = await fetch(`/api/metodos-ensino/${confirmDelete.id}`, { method: "DELETE" });
      if (!res.ok) {
        const data = await res.json();
        setErroDelete(data.erro ?? "Erro ao excluir.");
        return;
      }
      setMetodos((prev) => prev.filter((m) => m.id !== confirmDelete.id));
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">
            {metodos.length} método{metodos.length !== 1 ? "s" : ""} cadastrado{metodos.length !== 1 ? "s" : ""}
          </p>
          <button
        ...
        {metodos.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-8">Nenhum método cadastrado ainda.</p>
        ) : (
            ...
            {metodos.map((m) => (
              <div key={m.id} className="flex items-center justify-between py-2.5">
                ...
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => { setErroEdit(""); setEditando({ ...m }); }}
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            <h2 className="text-lg font-bold text-slate-800 mb-4">Novo método de ensino</h2>
            <div>
      ...
      {editando && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            <h2 className="text-lg font-bold text-slate-800 mb-4">Editar método</h2>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Nome *</label>
              <input
                value={editando.nome}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            ...
            <p className="text-sm text-slate-600">
              Tem certeza que deseja excluir o método <strong>{confirmDelete.nome}</strong>?
            </p>
            {erroDelete && <p className="text-sm text-red-600 bg-red-50 rounded-lg px-3 py-2 mt-3">{erroDelete}</p>}
            <div className="flex gap-3 mt-5">
              <button
                onClick={excluirMetodo}
                disabled={salvando}
