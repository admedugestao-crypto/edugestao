...
export default async function ConteudosPage() {
  ...
  const [alunos, conteudos, professoras, materias] = await Promise.all([
    prisma.aluno.findMany({
      ...
      include: {
        // Planejamento sem aula usa as disciplinas do cadastro do aluno.
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}><h1>Conteúdos</h1><p>Planeje a próxima aula e acompanhe o que cada aluno aprendeu.</p></header>
      <Suspense>
      <ConteudosClient
        variant="v2"
        alunos={alunos.map((a) => ({
