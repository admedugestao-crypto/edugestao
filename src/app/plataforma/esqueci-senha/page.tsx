...
export default function PlataformaEsqueciSenhaPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setCarregando(true);
    try {
      const res = await fetch("/api/plataforma/esqueci-senha", {
        method: "POST",
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 text-sm mt-1">
            Acesso interno da plataforma — informe seu e-mail para receber um link de redefinição.
          </p>
        </div>
        ...
        {enviado ? (
          <div className="space-y-4">
            <p className="text-emerald-700 text-sm bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">
              Se este e-mail estiver cadastrado na plataforma, você vai receber um link de redefinição em instantes.
            </p>
            {linkDev && (
