...
function motor(tipo,data,status='REALIZADA',pago=false,reposicao=false){
 ...
 const prisma={agendaAula:{findFirst:async()=>({id:'aula',alunoId:'aluno',data:new Date(data),status,reposicao})},aluno:{findUnique:async()=>aluno},pagamentoAula:{findFirst:async()=>pago?{pagamentoId:'p',pagamento:{parcela:1,pago:true,valorCobrado:99}}:null,create:async()=>({})},pagamento:{aggregate:async()=>({_max:{parcela:0}}),create:async({data})=>{created.push(data);return {id:'p',...data}},update:async()=>{updates++;throw Error('Quitado alterado')}}};
 ...
 const helperCode=esbuild.transformSync(fs.readFileSync('src/lib/aulaElegivelCobranca.ts','utf8'),{loader:'ts',format:'cjs'}).code;
 vm.runInNewContext('(function(module,exports){'+helperCode+'})',{})(helper,helper.exports);
 vm.runInNewContext('(function(require,module,exports){'+code+'})',{Date,Number,console})(id=>{
  if(id==='@/lib/prisma')return {prisma};
  if(id==='@/lib/aulaElegivelCobranca')return helper.exports;
  throw Error('Dependência não simulada: '+id);
 },mod,mod.exports);
...
for(const [tipo,data,venc] of [['MENSAL','2026-09-08','2026-09-10'],['QUINZENAL','2026-09-08','2026-09-10'],['QUINZENAL','2026-09-22','2026-09-25'],['SEMANAL','2026-09-30','2026-10-05'],['POR_AULA','2026-09-08','2026-09-10']]){
 test(`${tipo} em ${data}: valor, vínculo, modalidade e vencimento`,async()=>{const m=motor(tipo,data);const r=await m.run();assert.equal(r.semCobranca,false);assert.equal(m.created.length,1);assert.equal(m.created[0].valorCobrado,150);assert.equal(m.created[0].tipoCobrancaGerada,tipo);assert.equal(m.created[0].dataVencimento.toISOString().slice(0,10),venc);m.aluno.tipoCobranca='OUTRA';assert.equal(m.created[0].tipoCobrancaGerada,tipo);});
}
...
test('motor não cria nem atualiza cobrança ao realizar reposição',async()=>{const m=motor('MENSAL','2026-09-22','REALIZADA',false,true);assert.equal((await m.run()).semCobranca,true);assert.equal(m.created.length,0);assert.equal(m.updates(),0);});
